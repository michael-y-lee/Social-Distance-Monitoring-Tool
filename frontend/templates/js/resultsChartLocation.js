class ResultsChartLocation {
    ...
    wrangleData () {
        ...
        vis.number_of_cameras_at_location = 6;

        //determine min frame count - aggregate based on lowest frame
        vis.totalFrameCount = 10000000000000;
        for (let i = 1; i < vis.number_of_cameras_at_location; i++) {
            vis.mwDataset = eval("mw"+i+"Raw");
            let this_clip_total_frames = (Object.keys(vis.mwDataset.maskStats).length);
            if (this_clip_total_frames < vis.totalFrameCount) {
                vis.totalFrameCount = this_clip_total_frames;
            }
        }

        console.log(vis.totalFrameCount)
        ...
        for (let i = 0; i < vis.totalFrameCount; i++) {
            vis.totalPeopleByFrame.push(0)
            vis.sdviolationsByFrame.push(0)
            vis.properMaskWearByFrame.push(0)
            vis.improperMaskWearByFrame.push(0)
            vis.noMaskWearByFrame.push(0)
        }

        //sum all the frame counts
        for (let j = 1; j < vis.number_of_cameras_at_location; j++) {
            vis.mwDataset = eval("mw"+j+"Raw");
            vis.sdDataset = eval("sd"+j+"Raw");
            //loop over data and append into arrays for plotting
            for (let i = 0; i < (vis.totalFrameCount); i++) {
                vis.totalPeopleByFrame[i] += vis.sdDataset.sdOutput.total_people[i]
                vis.sdviolationsByFrame[i] += vis.sdDataset.sdOutput.num_violation[i]
                vis.properMaskWearByFrame[i] += vis.mwDataset.maskStats[(i+1).toString()][1] //number of masks
                vis.improperMaskWearByFrame[i] += vis.mwDataset.maskStats[(i+1).toString()][3] //number of people improperly wearing masks
                vis.noMaskWearByFrame[i] += (vis.totalPeopleByFrame[i] - vis.properMaskWearByFrame[i] - vis.improperMaskWearByFrame[i])
            }
        }

        //loop over each frame and get average
        for (let i = 0; i < (vis.totalFrameCount); i++) {
            vis.totalPeopleByFrame[i] = vis.totalPeopleByFrame[i] / vis.number_of_cameras_at_location;
            vis.sdviolationsByFrame[i] = vis.sdviolationsByFrame[i] / vis.number_of_cameras_at_location;
            vis.properMaskWearByFrame[i] = vis.properMaskWearByFrame[i] / vis.number_of_cameras_at_location;
            vis.improperMaskWearByFrame[i] = vis.improperMaskWearByFrame[i] / vis.number_of_cameras_at_location;
            vis.noMaskWearByFrame[i] = vis.noMaskWearByFrame[i] / vis.number_of_cameras_at_location;
        }
